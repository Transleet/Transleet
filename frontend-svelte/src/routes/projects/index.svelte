<script lang="ts">
	...
	async function removeProject() {
		if (projects.length > 0) {
			let projectToBeRemoved = projects.shift().key;
			connection.invoke('Delete', projectToBeRemoved);
			projects = projects;
		}
	}
</script>

<div class="overflow-x-auto max-w-full pl-4 pr-4 ml-2 mr-2">
	<div class="pl-4">
		<h class="p-2 text-2xl">Projects</h>
		<br />
		<button class="btn p-2" on:click={createProject}>New Project</button>
		<button class="btn p-2" on:click={removeProject}>Remove project</button>
	</div>
		...
		{#each projects as project}
			<li>
				<div class="card w-60 p-2">
					...
					<figure><img src={project.avatar ?? '/no_image.jpg'} alt="No" /></figure>
					<div class="card-body">
